		...
		unsigned long start, unsigned long end, int *locked)
{
	...
	VM_BUG_ON(!MM_PAGE_ALIGNED(mm, start));
	VM_BUG_ON(!MM_PAGE_ALIGNED(mm, end));
...
size_t fault_in_writeable(char __user *uaddr, size_t size)
{
	...
	unsigned long page_size = MM_PAGE_SIZE(current->mm);

	if (unlikely(size == 0))
		return 0;
	if (!user_write_access_begin(uaddr, size))
		return size;
	if (!MM_PAGE_ALIGNED(current->mm, uaddr)) {
		unsafe_put_user(0, uaddr, out);
		uaddr = (char __user *)ALIGN((unsigned long)uaddr, page_size);
	}
	end = (char __user *)ALIGN((unsigned long)start + size, page_size);
...
size_t fault_in_safe_writeable(const char __user *uaddr, size_t size)
{
	...
	if (unlikely(size == 0))
		return 0;
	end = MM_PAGE_ALIGN(mm, start + size);
	if (end < start)
		end = 0;
...
size_t fault_in_readable(const char __user *uaddr, size_t size)
{
	...
	if (!MM_PAGE_ALIGNED(current->mm, uaddr)) {
		...
		uaddr = (const char __user *)ALIGN((unsigned long)uaddr, page_size);
	}
	end = (const char __user *)ALIGN((unsigned long)start + size, page_size);
	if (unlikely(end < start))
		end = NULL;
	while (uaddr != end) {
		unsafe_get_user(c, uaddr, out);
...
/* Bound whole-range repinning after lock drops, including the first attempt. */
#define GUP_COMPAT_MAX_ATTEMPTS	16

/*
 * Keep VMA geometry stable from GUP through span construction. The backing
 * slice is fixed by the VMA while mmap_lock is held; a later COW may replace
 * the page, but cannot change the offset within the page we pinned.
			   ...
			   bool pin)
{
	...
	if (locked) {
		...
		ret = -EAGAIN;
		for (attempt = 0; attempt < GUP_COMPAT_MAX_ATTEMPTS; attempt++) {
			if (attempt) {
				cond_resched();
				if (mmap_read_lock_killable(mm)) {
					ret = -EINTR;
			...
			ret = -EAGAIN;
			if (gup_signal_pending(gup_flags)) {
				ret = -EINTR;
				break;
			}
		}
 ...
 * Compat callers return -EAGAIN if repeated mmap_lock drops exhaust the
 * whole-range repinning budget.
 */
