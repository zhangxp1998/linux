			   ...
			   struct mm_walk *args)
{
	...
	pte = pte_offset_map_rw_nolock(mm, pmd, start & MM_PMD_MASK(mm), &pmdval, &ptl);
	if (!pte)
		return false;
...
restart:
	for (i = pte_index(start), addr = start; addr != end;
		...
		if (folio_test_large(folio)) {
			const unsigned int max_nr = (end - addr) >> MM_PAGE_SHIFT(mm);
	...
	if (i < MM_PTRS_PER_PTE(mm) &&
	    get_next_vma(MM_PMD_MASK(mm), MM_PAGE_SIZE(mm), args, &start, &end))
		goto restart;
