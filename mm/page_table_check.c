...
void __page_table_check_pmd_clear(struct mm_struct *mm, pmd_t pmd)
{
	...
	if (pmd_user_accessible_page(pmd)) {
		page_table_check_clear_slices(pmd_pfn(pmd),
...
void __page_table_check_pud_clear(struct mm_struct *mm, pud_t pud)
{
	...
	if (pud_user_accessible_page(pud)) {
		page_table_check_clear_slices(pud_pfn(pud),
		...
		unsigned int nr)
{
	...
	if (pmd_user_accessible_page(pmd))
		page_table_check_set_slices(pmd_pfn(pmd), stride * nr, 0,
					    PPPS_SLICES_PER_PAGE,
					    pmd_write(pmd));
}
		...
		unsigned int nr)
{
	...
	if (pud_user_accessible_page(pud))
		page_table_check_set_slices(pud_pfn(pud), stride * nr, 0,
					    PPPS_SLICES_PER_PAGE,
					    pud_write(pud));
}
