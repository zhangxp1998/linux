...
static inline bool vmg_can_merge_offsets(const struct vma_merge_struct *vmg,
					 bool merge_next)
{
	bool sliced = ppps_mm_is_compat(vmg->mm) &&
		(vmg->file || (vmg->vm_flags & VM_SHARED) ||
		 (vmg->middle && ppps_vma_has_slices(vmg->middle)) ||
		 (vmg->copied_from && ppps_vma_has_slices(vmg->copied_from)));
