...
static struct folio *ppps_uffd_move_prealloc(struct mm_struct *mm,
...
static long ppps_uffd_move_slice(struct ppps_uffd_move *op)
{
	...
	struct folio *prealloc = op->prealloc;
		...
		pte_t orig_src_pte)
{
	...
	/* Allocation must precede the PTE mappings' RCU read sections. */
	op.prealloc = ppps_uffd_move_prealloc(op.mm, dst_vma, dst_addr);
	if (!op.prealloc)
		return -ENOMEM;
