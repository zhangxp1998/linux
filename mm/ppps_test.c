...
static int tuple_test_init(struct kunit *test)
{
	...
	ctx->source->mapping = (void *)FOLIO_MAPPING_ANON;
	ctx->destination->mapping = (void *)FOLIO_MAPPING_ANON;
...
static void vma_merge_slice_offset_test(struct kunit *test)
{
	...
	struct tuple_test *ctx = test->priv;
	struct vm_area_struct middle, next;
	...
	vma_init(&middle, ctx->mm);
	middle.vm_ops = &sliced_ops;
...
static void page_table_check_nonleaf_input_test(struct kunit *test)
{
	...
	__page_table_check_pmds_set(ctx->mm, pmds, __pmd(0), ARRAY_SIZE(pmds));
	__page_table_check_puds_set(ctx->mm, puds, __pud(0), ARRAY_SIZE(puds));

	/* pfn_pmd()/pmd_pfn() require native PMD physical alignment. */
	order = get_order(PMD_SIZE);
	...
	__page_table_check_pmds_set(ctx->mm, pmds, huge_pmd, 1);
