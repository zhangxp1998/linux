				 ...
				 unsigned long dst_addr)
{
	...
	offset = vma_file_offset(dst_vma) + dst_addr - dst_vma->vm_start;
					 ...
					 unsigned long dst_addr)
{
	...
	ret = mfill_atomic_install_pte(dst_pmd, dst_vma, dst_addr,
				       &folio->page, true, vma_address_to_slice(dst_vma, dst_addr), 0);
	if (ret)
		goto out_put;
				     ...
				     uffd_flags_t flags)
{
	...
	ret = mfill_atomic_install_pte(dst_pmd, dst_vma, dst_addr,
				       page, false, vma_address_to_slice(dst_vma, dst_addr), flags);
	if (ret)
		goto out_release;
...
retry:
	...
	if (!ppps_vma_validate_uffd_alignment(dst_vma, dst_start, dst_start + len)) {
		err = -EINVAL;
		goto out_unlock;
	}
