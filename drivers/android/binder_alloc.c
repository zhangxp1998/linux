					     ...
					     unsigned long address)
{
	return alloc->vm_start +
		ALIGN_DOWN(address - alloc->vm_start, PAGE_SIZE);
}

static unsigned long binder_alloc_page_end(struct binder_alloc *alloc,
					   unsigned long address)
{
	return alloc->vm_start + PAGE_ALIGN(address - alloc->vm_start);
}
			      ...
			      struct page *page)
{
	...
	if (vma) {
		if (binder_alloc_is_mapped(alloc))
			ret = vm_insert_pages(vma, addr, &page, &num);
		vma_end_read(vma);
		return ret;
	}
			      ...
			      struct vm_area_struct *vma)
{
	...
	alloc->pages = kvzalloc_objs(alloc->pages[0],
				     DIV_ROUND_UP(alloc->buffer_size, PAGE_SIZE));
	if (!alloc->pages) {
		ret = -ENOMEM;
