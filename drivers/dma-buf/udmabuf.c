...
static int vmap_udmabuf(struct dma_buf *buf, struct iosys_map *map)
{
	...
	first_offset = ubuf->spans[0].offset;
	...
	for (i = 0; i < ubuf->pagecount; i++) {
		...
		unsigned long source_offset = ubuf->spans[i].offset;
				     ...
				     enum dma_data_direction direction)
{
	...
	ret = sg_alloc_table_from_page_slices(sg, ubuf->pages, ubuf->spans,
					      ubuf->pagecount, ubuf->size,
					      GFP_KERNEL);
	if (ret < 0)
		goto err_alloc;
...
static __always_inline int init_udmabuf(struct udmabuf *ubuf, pgoff_t pgcnt)
{
	...
	ubuf->spans = kvmalloc_array(pgcnt, sizeof(*ubuf->spans), GFP_KERNEL);
	if (!ubuf->spans)
		return -ENOMEM;
			       ...
			       loff_t start, loff_t size, struct folio **folios)
{
	...
	for (cur_folio = 0; cur_folio < nr_folios; ++cur_folio) {
		...
		for (; subpgoff < fsize; subpgoff += unit_size) {
			ubuf->pages[upgcnt] =
				folio_page(folios[cur_folio], subpgoff >> PAGE_SHIFT);
			ubuf->spans[upgcnt].offset = subpgoff & ~PAGE_MASK;
			ubuf->spans[upgcnt].length = unit_size;
			++upgcnt;
