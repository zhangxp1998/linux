		     ...
		     struct frame_vector *vec)
{
	...
	if (ppps_mm_is_compat(mm)) {
		...
		for (i = 0; i < nr_frames; i++, addr += vec->frame_size) {
			...
			vec->offsets[i] = vma_address_to_slice(vma, addr) *
					  vec->frame_size;
			if (!i)
				vec->offsets[i] += first_offset;
		}
...
unlock:
		...
		memset(vec->offsets, 0, nr_frames * sizeof(*vec->offsets));
		vec->offsets[0] = offset_in_page(start);
...
struct frame_vector *frame_vector_create(unsigned int nr_frames)
{
	...
	if (WARN_ON_ONCE(nr_frames > INT_MAX / sizeof(void *) / 2))
		return NULL;
	if (check_mul_overflow((size_t)nr_frames, sizeof(*vec->offsets),
			       &offsets_size) ||
