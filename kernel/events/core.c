...
static void perf_event_init_userpage(struct perf_event *event)
{
	...
	userpg->data_offset = rb->mmap_page_size;
...
static bool perf_mmap_is_aux(struct perf_buffer *rb,
			     struct vm_area_struct *vma)
{
	return rb_has_aux(rb) && vma_file_offset(vma) == rb->aux_offset;
}
...
static void perf_mmap_open(struct vm_area_struct *vma)
{
	...
	if (perf_mmap_is_aux(event->rb, vma))
...
static vm_fault_t perf_mmap_pfn_mkwrite(struct vm_fault *vmf)
{
	...
	if (vma_file_offset(vmf->vma) ||
	    vmf->address >= vmf->vma->vm_start +
	    MM_PAGE_SIZE(vmf->vma->vm_mm))
...
static int map_range_ppps(struct perf_buffer *rb, struct vm_area_struct *vma)
{
	...
	page = perf_mmap_to_page(rb, 0);
	if (!page) {
		err = -EINVAL;
		goto out;
	}
	err = remap_pfn_range_slice(vma, uaddr, page_to_pfn(page), 0,
				    page_size, prot);
	if (err)
		goto out;
	uaddr += page_size;
	...
	while (data_offset < data_size) {
		...
		page = perf_mmap_to_page(rb, 1 + data_offset / PAGE_SIZE);
		if (!page) {
			err = -EINVAL;
			goto out;
		}
		err = remap_pfn_range_slice(vma, uaddr, page_to_pfn(page), slice,
					    page_size, prot);
		if (err)
			goto out;
		uaddr += page_size;
...
static int map_aux_range_ppps(struct perf_buffer *rb,
			      struct vm_area_struct *vma)
{
	unsigned long page_size = MM_PAGE_SIZE(vma->vm_mm);
	...
	while (offset < size) {
		...
		struct page *page = virt_to_page(rb->aux_pages[offset / PAGE_SIZE]);
...
static int map_range(struct perf_buffer *rb, struct vm_area_struct *vma)
{
	...
	if (ppps_mm_is_compat(vma->vm_mm)) {
		u64 mmap_offset = vma_file_offset(vma);

		if (!mmap_offset)
			return map_range_ppps(rb, vma);
		if (rb_has_aux(rb) && mmap_offset == rb->aux_offset)
			return map_aux_range_ppps(rb, vma);
	}
	...
	if (err)
		zap_page_range_single(vma, vma->vm_start, vma_size, NULL);
#endif
...
static int perf_mmap_rb(struct vm_area_struct *vma, struct perf_event *event,
			unsigned long nr_pages, unsigned long data_size)
{
	...
	if (event->rb) {
		if (data_page_nr(event->rb) != nr_pages ||
		    ...
		    event->rb->mmap_page_size != MM_PAGE_SIZE(vma->vm_mm))
			return -EINVAL;
	...
	rb = rb_alloc(nr_pages, data_size,
	...
	rb->mmap_page_size = MM_PAGE_SIZE(vma->vm_mm);
...
{
	long extra = 0, user_extra = nr_pages;
	u64 aux_offset, aux_size, mmap_offset;
	struct perf_buffer *rb;
	int ret, rb_flags = 0;
	...
	if (aux_offset < perf_data_size(rb) + rb->mmap_page_size)
		return -EINVAL;
...
{
	struct perf_event *event = file->private_data;
	unsigned long vma_size, nr_pages, data_size;
	u64 mmap_offset;
	mapped_f mapped;
	int ret;
	...
	/*
	 * A PPPS main ring starts with one process-page metadata region and uses
	 * native backing pages for its process-page-sized logical data ring. AUX
	 * rings have an externally supplied byte offset and native-page-sized
	 * backing storage.
	 */
	if (!mmap_offset && ppps_mm_is_compat(vma->vm_mm)) {
		unsigned long page_size = MM_PAGE_SIZE(vma->vm_mm);

		if (vma_size < page_size)
			return -EINVAL;
		data_size = vma_size - page_size;
		if (data_size && !is_power_of_2(data_size))
			return -EINVAL;
		nr_pages = DIV_ROUND_UP(data_size, PAGE_SIZE) + 1;
	} else {
		nr_pages = vma_size / PAGE_SIZE;
		if (mmap_offset &&
		    (!nr_pages || vma_size != PAGE_SIZE * nr_pages))
			return -EINVAL;
		if (!nr_pages)
			nr_pages = 1;
		data_size = (nr_pages - 1) * PAGE_SIZE;
	}
	...
	scoped_guard (mutex, &event->mmap_mutex) {
		...
		if (!mmap_offset)
			ret = perf_mmap_rb(vma, event, nr_pages, data_size);
		else
...
{
	unsigned long vma_size = vma->vm_end - vma->vm_start;
	unsigned long off = vma_file_offset(vma);
	struct file *file = vma->vm_file;
