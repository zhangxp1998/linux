...
#define TRANSFER_SIZE	(2 * USER_PAGE_SIZE)
#define MAPPING_SIZE	(16 * USER_PAGE_SIZE)
#define FILE_MAPPING_OFFSET	(3 * USER_PAGE_SIZE)
...
static bool pwrite_full(int fd, const void *buffer, size_t length, off_t offset)
{
	const unsigned char *pos = buffer;
...
static int run_child(int info_fd, int command_fd)
{
	...
	target = (unsigned char *)align_up((uintptr_t)mapping, LARGE_PAGE_SIZE);
	if (target + TRANSFER_SIZE > mapping + MAPPING_SIZE)
		return EXIT_FAILURE;
	memset(target, 0x11, USER_PAGE_SIZE);
	memset(target + USER_PAGE_SIZE, 0x22, USER_PAGE_SIZE);
	...
	memset(file_contents, 0x55, USER_PAGE_SIZE);
	memset(file_contents + USER_PAGE_SIZE, 0x66, USER_PAGE_SIZE);
	...
	passed = buffer_is_value(target, USER_PAGE_SIZE, 0x33) &&
		 buffer_is_value(target + USER_PAGE_SIZE, USER_PAGE_SIZE, 0x44) &&
		 buffer_is_value(file_mapping, USER_PAGE_SIZE, 0x77) &&
		 buffer_is_value(file_mapping + USER_PAGE_SIZE, USER_PAGE_SIZE,
				 0x88);
...
static int run_parent(void)
{
	...
	ksft_test_result(info.page_size == USER_PAGE_SIZE &&
			 !(info.anon_address & (LARGE_PAGE_SIZE - 1)),
	...
	read_data_ok = buffer_is_value(read_buffer, USER_PAGE_SIZE, 0x11) &&
		       buffer_is_value(read_buffer + USER_PAGE_SIZE,
				       USER_PAGE_SIZE, 0x22);
	ksft_test_result(read_data_ok,
			 "process_vm_readv preserves anonymous 4K page contents\n");

	memset(write_buffer, 0x33, USER_PAGE_SIZE);
	memset(write_buffer + USER_PAGE_SIZE, 0x44, USER_PAGE_SIZE);
	...
	read_data_ok = buffer_is_value(read_buffer, USER_PAGE_SIZE, 0x55) &&
		       buffer_is_value(read_buffer + USER_PAGE_SIZE,
				       USER_PAGE_SIZE, 0x66);
	ksft_test_result(read_data_ok,
			 "process_vm_readv honors the file VMA slice offset\n");

	memset(write_buffer, 0x77, USER_PAGE_SIZE);
	memset(write_buffer + USER_PAGE_SIZE, 0x88, USER_PAGE_SIZE);
	local.iov_base = write_buffer;
	local.iov_len = sizeof(write_buffer);
	errno = 0;
	transferred = process_vm_writev(child, &local, 1, &remote, 1, 0);
