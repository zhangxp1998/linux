...
static int run_test(void)
{
	...
	fd = open("/dev/" DMA_MMAP_PAGES_PPPS_DEVICE_NAME,
		  O_RDWR | O_CLOEXEC);
	ksft_test_result(fd >= 0, "open the dma_mmap_pages test device\n");
	if (fd < 0)
	...
	ksft_test_result(mapping == reservation,
			 "map only the first 4K slice with dma_mmap_pages\n");
	if (mapping == MAP_FAILED)
		ksft_exit_fail_msg("dma_mmap_pages mmap failed: %s\n",
				   strerror(errno));
	...
	ksft_test_result(guards_fault,
			 "dma_mmap_pages does not populate the three guard slices\n");
...
static int exec_compat(void)
{
	...
	if (personality(persona | ADDR_4KB_COMPAT_PAGE_SIZE) < 0)
		ksft_exit_fail_msg("personality set failed: %s\n",
				   strerror(errno));
	execl("/proc/self/exe", "dma_mmap_pages_ppps", "--run", NULL);
