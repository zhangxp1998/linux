...
static int run_test(void)
{
	...
	struct io_uring_params user_params = {
		.flags = IORING_SETUP_NO_MMAP | IORING_SETUP_DEFER_TASKRUN |
			 IORING_SETUP_SINGLE_ISSUER,
	};
	struct io_uring_params resize_params = {
		.sq_entries = 4,
	};
	...
	void *resize_ring = MAP_FAILED;
	void *resize_sqes = MAP_FAILED;
	off_t pbuf_offset;
	bool mapped = false;
	...
	if (user_fd >= 0) {
		...
		user_ring_completed = submit_nop(user_fd, &user_params, &user_map);
	} else {
		ksft_print_msg("user-backed ring setup failed: %s\n",
			       strerror(errno));
	}
	resize_ring = mmap(NULL, USER_PAGE_SIZE, PROT_READ | PROT_WRITE,
			   MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	resize_sqes = mmap(NULL, USER_PAGE_SIZE, PROT_READ | PROT_WRITE,
			   MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	if (user_fd >= 0 && user_ring_completed &&
	    resize_ring != MAP_FAILED && resize_sqes != MAP_FAILED) {
		resize_params.cq_off.user_addr = (uintptr_t)resize_ring;
		resize_params.sq_off.user_addr = (uintptr_t)resize_sqes;
		ret = register_ring(user_fd, IORING_REGISTER_RESIZE_RINGS,
				    &resize_params, 1);
		user_ring_resized = ret == 0;
	}
	if (user_fd >= 0 && user_ring_completed && !user_ring_resized)
		ksft_print_msg("user-backed ring resize failed: %s\n",
			       strerror(errno));
	if (user_ring_resized) {
		user_map.sq_ring = resize_ring;
		user_map.cq_ring = resize_ring;
		user_map.sqes = resize_sqes;
		user_ring_resized = submit_nop(user_fd, &resize_params, &user_map);
	}
	ksft_test_result(user_fd >= 0 && user_ring_completed && user_ring_resized,
			 "create, resize, and use 4K user-backed SQ/CQ/SQE regions\n");
	if (user_fd >= 0)
		close(user_fd);
	if (fd >= 0)
		close(fd);
	...
	if (resize_sqes != MAP_FAILED)
		munmap(resize_sqes, USER_PAGE_SIZE);
	if (resize_ring != MAP_FAILED)
		munmap(resize_ring, USER_PAGE_SIZE);
