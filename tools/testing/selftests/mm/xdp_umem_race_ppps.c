...
#define XDP_PIN_DRAIN_IOCTL _IO(0xa4, 2)
...
static int wait_unpinned(int probe_fd, void *stable_a, void *stable_b)
{
	...
	/* close() defers AF_XDP destruction through RCU and system_wq. */
	if (ioctl(probe_fd, XDP_PIN_DRAIN_IOCTL))
		return -1;
	...
	fprintf(stderr,
		"PIN_CLEANUP_TIMEOUT pinned=%u,%u refs=%lld,%lld pfn=%llu,%llu\n",
		probe.pinned[0], probe.pinned[1],
...
static int run_test(void)
{
	...
	unsigned int pending_cleanup = 0;
	...
	*(unsigned char *)stable_b = 0xb2;
	...
	for (i = 0; i < MAX_ATTEMPTS; i++) {
		...
		if (fd >= 0) {
			...
			close(fd);
			pending_cleanup++;
			if (pending_cleanup == 256 &&
			    wait_unpinned(probe_fd, stable_a, stable_b))
	...
	atomic_store(&context.stop, true);
	pthread_join(toggler, NULL);
	if (pending_cleanup && !cleanup_error)
		cleanup_error = wait_unpinned(probe_fd, stable_a, stable_b);
