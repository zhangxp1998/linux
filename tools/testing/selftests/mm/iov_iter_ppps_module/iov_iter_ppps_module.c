...
static struct miscdevice iov_iter_ppps_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = IOV_ITER_PPPS_DEVICE_NAME,
	.fops = &iov_iter_ppps_fops,
};

static int __init iov_iter_ppps_init(void)
{
	return misc_register(&iov_iter_ppps_device);
}

static void __exit iov_iter_ppps_exit(void)
{
	misc_deregister(&iov_iter_ppps_device);
}

module_init(iov_iter_ppps_init);
module_exit(iov_iter_ppps_exit);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("PPPS user-backed iov_iter page extraction regression helper");
