...
static int run_test(void)
{
	...
	ksft_test_result(fd >= 0, "open the selinuxfs enforce control\n");
	if (fd < 0)
	...
	saved_errno = errno;
	ksft_print_msg("enforce write returned %zd, errno %d (%s)\n",
		       written, saved_errno, strerror(saved_errno));
	ksft_test_result(written == -1 && saved_errno == ENOMEM,
			 "bound selinuxfs writes to the process page\n");
...
static int exec_compat(void)
{
	...
	if (personality(persona | ADDR_4KB_COMPAT_PAGE_SIZE) < 0)
		ksft_exit_fail_msg("personality set failed: %s\n",
				   strerror(errno));
	execl("/proc/self/exe", "selinuxfs_write_size_ppps", "--run", NULL);
