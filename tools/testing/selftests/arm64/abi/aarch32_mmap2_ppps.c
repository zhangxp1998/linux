...
/* Read-only files of at least 16K that exist on Android or a busybox guest. */
static const char *const backing_paths[] = {
	"/system/bin/toybox",
	"/system/bin/sh",
	"/bin/busybox",
	"/bin/sh",
...
void _start(void)
{
	...
	fd = -1;
	for (i = 0; backing_paths[i] && fd < 0; i++) {
		fd = syscall3(__NR_open, (long)backing_paths[i], O_RDONLY, 0);
		if (fd >= 0 && syscall3(__NR_lseek, fd, 4 * PAGE_SIZE_4K, 0) < 0) {
