...
static int run_test(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(paths); i++) {
		...
		saved_errno = errno;
		ksft_print_msg("%s write returned %zd, errno %d (%s)\n",
			       paths[i], written, saved_errno,
			       strerror(saved_errno));
		ksft_test_result(written == -1 && saved_errno == EINVAL,
				 "bound trace event input to the process page\n");
		close(fd);
	}
...
static int exec_compat(void)
{
	...
	if (personality(persona | ADDR_4KB_COMPAT_PAGE_SIZE) < 0)
		ksft_exit_fail_msg("personality set failed: %s\n",
				   strerror(errno));
	execl("/proc/self/exe", "trace_event_write_size_ppps", "--run", NULL);
