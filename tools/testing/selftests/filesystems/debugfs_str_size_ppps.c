...
static int run_test(void)
{
	...
	fd = open("/sys/kernel/debug/ppps_debugfs_str/value",
		  O_WRONLY | O_CLOEXEC);
	ksft_test_result(fd >= 0, "open a debugfs string value\n");
	if (fd < 0)
	...
	saved_errno = errno;
	ksft_print_msg("debugfs write returned %zd, errno %d (%s)\n",
		       written, saved_errno, strerror(saved_errno));
	ksft_test_result(written == -1 && saved_errno == E2BIG,
			 "bound a debugfs string to the process page\n");
...
static int exec_compat(void)
{
	...
	if (personality(persona | ADDR_4KB_COMPAT_PAGE_SIZE) < 0)
		ksft_exit_fail_msg("personality set failed: %s\n",
				   strerror(errno));
	execl("/proc/self/exe", "debugfs_str_size_ppps", "--run", NULL);
