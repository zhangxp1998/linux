...
void task_mem(struct seq_file *m, struct mm_struct *mm)
{
	...
	text = ALIGN(mm->end_code, PAGE_SIZE) - (mm->start_code & PAGE_MASK);
			 ...
			 unsigned long *data, unsigned long *resident)
{
	...
	*shared = __page_size_count(
		...
		PAGE_SHIFT);
	*text = (__PAGE_ALIGN(mm->end_code) -
		 (mm->start_code & __PAGE_MASK))
						>> __PAGE_SHIFT;
	*data = __page_size_count(mm->data_vm + mm->stack_vm);
	*resident = *shared + __page_size_count(
		get_mm_counter_sum_bytes(mm, MM_ANONPAGES) >> PAGE_SHIFT);
		...
		bool locked, bool present, int precise_mapcount)
{
	...
	unsigned long step_size = compound ? PAGE_SIZE_KERNEL : page_size;
	bool exclusive;
	int mapcount;
		...
		struct mm_walk *walk, int precise_mapcount)
{
	...
	if (pte_present(ptent)) {
		...
		if (!non_swap_entry(swpent)) {
			...
			mss->swap += page_size;
			mapcount = ppps_swap_mapcount(swpent);
			if (mapcount >= 2) {
...
static void show_smap_vma_flags(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	if (pad_pages)
		seq_printf(m, "pad=%lukB", pad_pages << (PAGE_SHIFT_KERNEL - 10));
			    ...
			    size_t count, loff_t *ppos)
{
	...
	int ret = 0, copied = 0;
	unsigned int nr_subpages = MM_UAPI_PAGE_SIZE(NULL) / PAGE_SIZE_KERNEL;
...
{
	return MM_UAPI_PAGE_SIZE(NULL) != PAGE_SIZE_KERNEL &&
	       file->f_op == &proc_pagemap_operations;
}
#ifdef CONFIG_ARM64_PER_PROCESS_PAGE_SIZE
/*
		...
		unsigned long end, struct mm_walk *walk, spinlock_t *ptl)
{
	...
	if (!vma->vm_file || ppps_mm_is_compat(vma->vm_mm) ||
	    !pmd_present(entry))
		return false;
	page = vm_normal_page_pmd(vma, addr, entry);
	if (IS_ERR_OR_NULL(page))
		return false;
	folio = page_folio(page);
	if (folio_test_anon(folio) || !folio_try_get(folio))
		return false;
	spin_unlock(ptl);

	if (folio_test_swapbacked(folio))
		mss->shmem_thp += end - addr;
	else if (!folio_is_zone_device(folio))
		mss->file_thp += end - addr;
	page += (addr & ~PMD_MASK) >> PAGE_SHIFT_KERNEL;
	for (; addr < end; addr += PAGE_SIZE_KERNEL, page++) {
		struct ppps_file_counts counts;
		...
		if (ppps_file_fast_counts(page, &counts) &&
			...
			if (counts.compat)
				...
				smaps_account(mss, PAGE_SIZE_KERNEL, page, false,
				     ...
				     unsigned long end, struct mm_walk *walk)
{
	...
	for (; addr != end; addr += page_size) {
		...
		enum ppps_file_snapshot_result result;

		result = ppps_file_pte_snapshot(vma, pmd, addr, &cache, &snapshot);
		if (result == PPPS_FILE_RETRY) {
			walk->action = ACTION_AGAIN;
			*mss = saved;
			break;
		}
		if (result == PPPS_FILE_GENERIC) {
			smaps_pte_entry(snapshot.ptep, addr, walk, -1);
			pte_unmap_unlock(snapshot.ptep, snapshot.ptl);
			continue;
		}
		if (ppps_mm_is_compat(vma->vm_mm)) {
			smaps_account(mss, page_size, snapshot.page, false,
				      pte_young(snapshot.entry), pte_dirty(snapshot.entry),
				      locked, true, snapshot.mapcount);
		} else {
			unsigned int slice;

			/* Each native PTE covers all slices, not just slice zero. */
			for (slice = 0; slice < PPPS_SLICES_PER_PAGE; slice++)
				smaps_account(mss, PAGE_SIZE_COMPAT, snapshot.page, false,
					      pte_young(snapshot.entry), pte_dirty(snapshot.entry),
					      locked, true,
				       ...
				       unsigned long end, struct mm_walk *walk)
{
	...
	for (; addr < end; addr += page_size) {
		...
		result = ppps_file_pte_snapshot(vma, pmdp, addr, &cache, &snapshot);
		if (result == PPPS_FILE_RETRY) {
			walk->action = ACTION_AGAIN;
			pm->pos = saved_pos;
			break;
		}
		pme = pte_to_pagemap_entry(pm, vma, addr, snapshot.entry,
					 snapshot.mapcount);
		if (result == PPPS_FILE_GENERIC)
			pte_unmap_unlock(snapshot.ptep, snapshot.ptl);
		err = add_to_pagemap(&pme, pm);
	...
	if (ptl) {
		...
		if (page)
			gather_stats(page, md, pmd_dirty(*pmd),
				     HPAGE_PMD_SIZE/PAGE_SIZE_KERNEL);
		spin_unlock(ptl);
		return 0;
	}
#endif
