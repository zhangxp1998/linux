...
{
	unsigned int nr_subpages = MM_PAGE_SIZE(mm) / PAGE_SIZE;
			 ...
			 unsigned long *data, unsigned long *resident)
{
	...
	*resident = mm_page_size_count(mm, native_shared + get_mm_counter_sum(mm, MM_ANONPAGES));
	return mm->total_vm;
}
		...
		bool locked, bool present, int precise_mapcount)
{
	...
	} else if (IS_ENABLED(CONFIG_NO_PAGE_MAPCOUNT)) {
	...
	for (i = 0; i < nr; i++, page++) {
		...
		if (IS_ENABLED(CONFIG_PAGE_MAPCOUNT) && precise_mapcount < 0) {
				     ...
				     unsigned long end, struct mm_walk *walk)
{
	...
	for (; addr != end; addr += page_size) {
		...
		smaps_account(mss, vma->vm_mm, page, false, young, dirty,
		...
		struct pagemapread *pm)
{
	...
	if (pmd_present(pmd)) {
		...
		if (pmd_swp_uffd_wp(pmd))
			flags |= PM_UFFD_WP;
		VM_WARN_ON_ONCE(!is_migration_entry(entry));
		page = pfn_swap_entry_to_page(entry);
	}
